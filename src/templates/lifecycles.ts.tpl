...
// @ts-ignore
import { setModelState } from './model';
...
export function genBootstrap(oldRender: typeof noop) {
  return async (props: any) => {
    const runtime = await getHuiRuntime();
    if (runtime.bootstrap) {
      await runtime.bootstrap(props);
    }
    render = oldRender;
  };
}
...
export function genUnmount(mountElementId: string) {
  return async (props: any) => {
    ...
    const runtime = await getHuiRuntime();
    if (runtime.unmount) await runtime.unmount(props);
  };
}
